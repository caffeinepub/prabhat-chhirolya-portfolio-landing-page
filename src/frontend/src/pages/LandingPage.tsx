...
export default function LandingPage({ mode, onModeChange }: LandingPageProps) {
  ...
  const [tappedMode, setTappedMode] = useState<
    "personal" | "professional" | null
  >(null);
  ...
  const remeasureTick = useModeSelectorRemeasure([
    personalButtonRef,
    professionalButtonRef,
  ]);
  ...
  const getSocialIcons = () => {
    if (mode === "professional") {
      return [
        {
          Icon: SiLinkedin,
          href: "https://www.linkedin.com/in/prabhatchhirolya",
          label: "LinkedIn",
          type: "brand" as const,
        },
        {
          Icon: SiGithub,
          href: "https://github.com/arprabhatchhirolya",
          label: "GitHub",
          type: "brand" as const,
        },
        {
          Icon: SiX,
          href: "https://x.com/_The_PC__",
          label: "X",
          type: "brand" as const,
        },
        {
          Icon: SiYoutube,
          href: "https://youtube.com/prabhatchhirolya",
          label: "YouTube",
          type: "brand" as const,
        },
        {
          Icon: Mail,
          href: "mailto:ar.prabhatchhirolya@gmail.com",
          label: "Email",
          type: "ui" as const,
        },
      ];
    ...
    if (mode === "personal") {
      return [
        {
          Icon: SiInstagram,
          href: "https://www.instagram.com/prabhatchhirolya.x?igsh=YmpnYWFlbnBtaDly",
          label: "Instagram",
          type: "brand" as const,
        },
        {
  ...
  const backgroundShift =
    hoveredMode === "personal" ? -2 : hoveredMode === "professional" ? 2 : 0;
  ...
  const shouldApplyDelayedFadeIn =
    mode === "initial" && isInitialLoad && showProfilePicture;

  // Determine which profile picture to show: only ONE at a time
  const showStaticProfilePicture =
    mode === "initial" &&
    profilePictureState === "center" &&
    !isReturningToLanding;
    ...
    <div className="relative h-full w-full">
      ...
      <div
        className={`absolute inset-0 z-10 flex items-center justify-center pointer-events-none ${
          mode === "initial" && !imageLoaded
            ? "opacity-0"
            : "identity-group-fade-in"
        }`}
      >
        <div
        ...
        >
          ...
          {showStaticProfilePicture && (
            <div
              className={`relative transition-all duration-500 ${
                !imageLoaded || !showProfilePicture
                  ? "opacity-0"
                  : shouldApplyDelayedFadeIn
                    ? "profile-picture-delayed-fade-in"
                    : ""
              }`}
              style={{
                width: "7rem",
          ...
          <div
            className={`flex flex-col px-4 items-center text-center ${
              mode === "initial" ? "gap-3 sm:gap-4" : "gap-2 sm:gap-3"
            }`}
          >
            <p
              className={`text-sm sm:text-base md:text-lg font-light tracking-wide text-foreground/70 transition-opacity duration-500 ${mode !== "initial" ? "opacity-0 h-0 overflow-hidden" : "opacity-100"}`}
            >
              Hey I'm…
            </p>
            <h1
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") handleReturnHome();
              }}
            ...
            <p className="max-w-2xl text-xs sm:text-sm md:text-base lg:text-lg font-light tracking-wide text-foreground/80">
              XR Designer • Immersive Visualiser • BIM/VDC Architect • AI-Native
              Builder
            </p>
          </div>
        </div>
      ...
      <footer className="absolute bottom-0 left-0 right-0 z-20 px-4 sm:px-8 md:px-12 py-4 sm:py-6 md:py-8">
        ...
        {/* Discreet admin link - visible only to those who know */}
        <a
          href="/admin"
          data-ocid="admin.link"
          className="absolute bottom-2 right-3 text-[10px] text-foreground/10 hover:text-foreground/25 transition-colors duration-300 font-light tracking-wider"
        >
          admin
        </a>
      </footer>
