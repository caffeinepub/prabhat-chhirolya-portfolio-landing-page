...
export default function ProfessionalModeContent({
  isFadingOut = false,
}: ProfessionalModeContentProps) {
  const [scrolled, setScrolled] = useState(false);
  const [showScrollCue, setShowScrollCue] = useState(true);
  const [activeRole, setActiveRole] = useState<number | null>(null);
  const [expandedEcosystem, setExpandedEcosystem] = useState<number | null>(
    null,
  );
  const [hoveredTool, setHoveredTool] = useState<string | null>(null);
  const [isBackgroundLoaded, setIsBackgroundLoaded] = useState(false);
  const { data: backendProfProjects } = useProfessionalProjects();
  const { data: backendWorkExperiences } = useWorkExperiences();
  const { data: backendToolCategories } = useToolCategories();
  const { data: backendWorkCards } = useWorkCards();
  ...
  useEffect(() => {
    const img = new Image();
    img.src =
      "/assets/generated/professional-mode-background.dim_1920x1080.jpg";
    img.onload = () => {
  ...
  const FALLBACK_WORK_PHILOSOPHY = [
    {
      title: "Building Before Knowing",
      description:
        "I start with prototypes and experiments rather than extensive planning. The best solutions emerge through iteration and direct engagement with the problem space.",
    },
    {
      title: "Systems Over Tools",
      description:
        "I focus on creating workflows and frameworks that scale beyond individual projects. Tools are temporary; good systems adapt and evolve.",
    },
    {
      title: "Experience as a Decision Tool",
      description:
        "Every design choice should be tested through user experience. I use immersive visualization and spatial interfaces to make complex decisions more intuitive.",
    },
    {
      title: "Future-Oriented by Default",
      description:
        "I design for emerging technologies and evolving workflows. Today's solutions should anticipate tomorrow's capabilities and constraints.",
    },
  ];
  const workPhilosophy =
    backendWorkCards && backendWorkCards.length > 0
      ? backendWorkCards.map((c) => ({
          title: c.title,
          description: c.description,
        }))
      : FALLBACK_WORK_PHILOSOPHY;
  ...
  const timeline = [
    ...
    },
    {
      ...
      vectorBg:
        "/assets/generated/academic-architecture-vectors.dim_800x400.png",
  ...
  const workExperience =
    backendWorkExperiences && backendWorkExperiences.length > 0
      ? backendWorkExperiences.map((exp) => ({
          company: exp.companyName,
          logo: exp.logoBlobId
            ? `https://blob.caffeine.ai/v1/blob/?blob_hash=${encodeURIComponent(exp.logoBlobId)}&owner_id=placeholder`
            : "/assets/jacobs.png",
          period: exp.roles.length > 0 ? exp.roles[0].dateRange : "",
          years: "",
          roles: exp.roles.map((r) => ({
            title: r.title,
            timeline: r.dateRange,
            level: 3,
  ...
  const FALLBACK_TOOL_CATEGORIES = [
    {
  ...
  const toolCategories =
    backendToolCategories && backendToolCategories.length > 0
      ? backendToolCategories.map((cat, i) => ({
  ...
  const FALLBACK_PROFESSIONAL_PROJECTS = [
    {
      title: "Google Randall – VR & Advanced Visualisation",
      description:
        "Ultra-realistic VR chalk walk and advanced visualization optimized for high-fidelity design review and executive walkthroughs",
      ...
      details: {
        ...
        role: "Project Lead",
        tools:
          "Revit, Sketchup, Enscape, Premierer Pro, Photoshop, HTC Vive VR headset",
        results:
          "Elevated design reviews with ultra-realistic VR, enabling faster feedback cycles and clearer stakeholder consensus.",
      },
    },
    {
      title: "University of Arizona, VIP Lounge- Adv Interactive VR",
      description:
        "Fully interactive VR experience with advanced user interactions, realistic lighting, and immersive spatial storytelling",
      ...
      details: {
        ...
        role: "Project Lead",
        tools:
          "Revit, Sketchup, Unreal Engine, Advanced Core Framework, Meta Quest VR headset",
        results:
          "Delivered an intuitive, high-immersion VR experience that improved spatial understanding and design confidence.",
      },
    },
    {
      title: "Genesis R&D Center – Durham",
      description:
        "Complete immersive visualization package combining cloud presentations, real-time rendering, and VR exploration.",
      ...
      details: {
        ...
        role: "Project Lead",
        tools:
          "Revit, Sketchup, Twinmotion & Web Presenter, Meta Quest VR headset",
        results:
          "Expanded stakeholder access and engagement through cloud-based and VR visualization across multiple platforms.",
      },
    },
    {
      title: "ASU CFL multiplayer collaborative VR",
      description:
        "Realistic, cross-platform collaborative XR enabling multi-user VR and non-VR design review across devices.",
      ...
      details: {
        ...
        results:
          "Enabled real-time, multi-user collaboration and design validation across platforms, transforming how stakeholders review and interact just like team calls.",
      },
    },
  ];
  const professionalProjects =
    backendProfProjects && backendProfProjects.length > 0
      ? backendProfProjects.map((p) => ({
          title: p.title,
          ...
          thumbnail: p.thumbnailBlobId
            ...
            : "/assets/GoogleRandalthumbnaill.jpg",
          details: {
      ...
      : FALLBACK_PROFESSIONAL_PROJECTS;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <section
          className={`min-h-[70vh] flex flex-col items-center justify-center text-center relative professional-mode-entry-bg ${isBackgroundLoaded ? "mode-bg-loaded" : ""}`}
        >
          ...
          <p className="max-w-3xl text-sm sm:text-base md:text-lg lg:text-xl font-light tracking-wide text-foreground/70 leading-relaxed mb-8 relative z-10">
            XR Designer & Immersive Visualiser working at the intersection of
            BIM/VDC, AECO, XR, and AI-driven workflows.
          </p>
        ...
        >
          ...
          <div className="w-full overflow-x-auto overflow-y-hidden scrollbar-hide -mx-6 sm:-mx-12 md:-mx-16 lg:-mx-24 px-6 sm:px-12 md:px-16 lg:px-24">
            <div
              ...
              className="flex gap-6 sm:gap-8 pb-4"
            >
              ...
              {timeline.map((stage, index) => (
                ...
                >
                  ...
                  <div className="group relative p-6 sm:p-8 rounded-lg border border-foreground/10 bg-card/20 backdrop-blur-sm transition-all duration-500 hover:border-accent-bright/30 hover:bg-card/30 hover:shadow-lg hover:shadow-accent-bright/5 overflow-hidden">
                    ...
                    <div
                      className={`absolute inset-0 opacity-20 ${stage.animationClass}`}
                    >
                      <img
                        src={stage.vectorBg}
                        alt=""
                  ...
                  <div className="space-y-8 sm:space-y-10">
                    {company.roles.map((role, roleIndex) => (
                      <div
                        ...
                        onMouseEnter={() =>
                          setActiveRole(companyIndex * 10 + roleIndex)
                        }
                      ...
                      >
                        ...
                        >
                          ...
                          <div className="mb-6">
                            ...
                            <ul className="space-y-2">
                              {role.responsibilities.map((resp) => (
                                ...
                                >
                                  <span className="text-accent-bright/60 flex-shrink-0 mt-1">
                                    •
                                  </span>
                                  <span>{resp}</span>
                                </li>
                              ...
                              <ul className="space-y-2">
                                {role.achievements.map((achievement) => (
                                  <li
                                    ...
                                    className="flex gap-2 text-xs sm:text-sm font-light tracking-wide text-foreground/70 leading-relaxed"
                                  >
                                    <span className="text-accent-bright flex-shrink-0 mt-1">
                                      ✓
                                    </span>
                          ...
                          <div className="mt-4 flex justify-center">
                            <div
                              className={`text-xs font-light tracking-wider text-foreground/40 transition-all duration-300 ${
                                activeRole === companyIndex * 10 + roleIndex
                                  ? "rotate-180"
                                  : ""
                              }`}
                      ...
                      <div
                        ...
                        ref={(el) => {
                          toolItemsRef.current[
                            categoryIndex * 10 + toolIndex + 1
                          ] = el;
                        }}
                        className="tool-item group relative"
                        onMouseEnter={() =>
                          setHoveredTool(`${categoryIndex}-${toolIndex}`)
                        }
                ...
                >
                  ...
                  >
                    ...
                    <div className="relative z-10 flex items-center justify-between mb-4">
                      <h4
                        className={`text-lg sm:text-xl font-light tracking-wide transition-colors duration-300 ${
                          expandedEcosystem === index
                            ? "text-accent-bright"
                            : "text-foreground group-hover:text-accent-bright"
                        }`}
                      >
                        {ecosystem.category}
                      </h4>
                    ...
                    >
                      <ul className="space-y-3 pt-2">
                        {ecosystem.items.map((item, itemIndex) => (
                          ...
                          >
                            <span className="text-accent-bright/80 flex-shrink-0 mt-1">
                              →
                            </span>
                            <span>{item}</span>
                          </li>
