...
2. **OG card image**: `https://www.prabhatchhirolya.com/assets/ogcard-v2.jpg`

## Verification Steps

### 1. Verify Image is Accessible

Open `https://www.prabhatchhirolya.com/assets/ogcard-v2.jpg` in your browser:
...
4. Expected: Large image preview with title, description, and ogcard-v2.jpg image (1200×630)
...
4. Expected: Large image card with title, description, and ogcard-v2.jpg image

**Note**: The versioned filename (ogcard-v2.jpg) helps bypass Twitter's aggressive caching. If you still see an old preview:
...
4. Expected: Preview with ogcard-v2.jpg image
...
1. **File missing from public directory**: The `ogcard-v2.jpg` file doesn't exist at `frontend/public/assets/ogcard-v2.jpg`
   - **Fix**: Ensure the file `ogcard-v2.jpg` (1200×630 JPEG) is physically present in `frontend/public/assets/` directory
   - **Source**: Copy from `assets/ogcard-v2.jpg` or `assets/generated/ogcard-v2.jpg.dim_1200x630.jpg`

2. **Not deployed to asset canister**: The file wasn't included in the IC asset canister deployment
   - **Fix**: Verify `.ic-assets.json5` includes the `assets/ogcard-v2.jpg` rule with `"ignore": false`
...
4. **Wrong path**: The file is in `assets/generated/` but not copied to the correct location
   - **Fix**: The file MUST be at `frontend/public/assets/ogcard-v2.jpg` (in the assets subdirectory)
...
- **Image not accessible**: Verify `https://www.prabhatchhirolya.com/assets/ogcard-v2.jpg` returns 200 and displays correctly.
- **Metadata missing**: Check that `index.html` contains all required OG and Twitter meta tags.
- **File location**: Ensure `ogcard-v2.jpg` exists at `frontend/public/assets/ogcard-v2.jpg`.

### Old Preview Showing
- The versioned filename (ogcard-v2.jpg) should bypass most caching issues
...
The OG card image (`ogcard-v2.jpg`) must meet these requirements:
...
- **Location**: `frontend/public/assets/ogcard-v2.jpg` (deployed to asset canister under /assets/)

## Deployment Checklist

**CRITICAL**: Before deploying, verify:
- [ ] `frontend/public/assets/ogcard-v2.jpg` exists and is a valid 1200×630 JPEG file
- [ ] `frontend/public/assets/generated/ogcard-v2.jpg.dim_1200x630.jpg` exists (source tracking)
- [ ] The file is the correct JPEG (not PNG renamed to .jpg)
- [ ] `.ic-assets.json5` includes `assets/ogcard-v2.jpg` with `"ignore": false` and explicit `Content-Type: image/jpeg` header
- [ ] `.well-known/ic-domains` lists both `prabhatchhirolya.com` and `www.prabhatchhirolya.com`
- [ ] `index.html` references `https://www.prabhatchhirolya.com/assets/ogcard-v2.jpg` (absolute URL with trailing slash on domain)
- [ ] All URLs (canonical, og:url, twitter:url) use consistent format: `https://www.prabhatchhirolya.com/`
- [ ] After deployment, test `https://www.prabhatchhirolya.com/assets/ogcard-v2.jpg` directly in browser (should show image, not 404)
...
- **Primary source**: `assets/ogcard-v2.jpg` (if it's already 1200×630 JPEG)
- **Generated source**: `assets/generated/ogcard-v2.jpg.dim_1200x630.jpg` (pre-sized version for tracking)
- **Deployment target**: `frontend/public/assets/ogcard-v2.jpg` (this is what gets deployed to the asset canister)

**Important**: The file in `frontend/public/assets/ogcard-v2.jpg` is the source-of-truth for deployment. It must be physically present in that directory before running `dfx deploy` or the build process.

## Cache Busting Strategy

The versioned filename (`ogcard-v2.jpg` instead of `ogcard.jpg`) is a cache-busting technique:
- Social media platforms cache OG images aggressively by URL
- Changing the filename forces platforms to fetch the new image
- If you need to update the image again in the future, increment the version: `ogcard-v3.jpg`, `ogcard-v4.jpg`, etc.
